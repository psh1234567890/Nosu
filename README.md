...
- Runtime env: `NODE_ENV=production`, `API_HOST=0.0.0.0`, `SERVE_STATIC_APP=true`, `ALLOWED_ORIGINS=https://nosu-best.onrender.com`

최초 데모 배포는 `AI_JUDGE_FORCE_LOCAL=true`와 파일 저장소로도 뜰 수 있지만, Render의 기본 파일 시스템은 재배포/재시작 시 유지되지 않을 수 있습니다. 실제 운영 데이터는 Supabase 값을 Render Environment에 넣고 `SUPABASE_STORAGE_MODE=normalized`로 전환하세요. `render.yaml`은 현재 기본 URL 기준으로 `ALLOWED_ORIGINS=https://nosu-best.onrender.com`을 넣습니다. 다른 서비스명이나 커스텀 도메인을 쓰면 Render Environment에서 실제 HTTPS 도메인으로 바꾸고 재배포합니다.
...
2. Render Environment에 `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_ANON_KEY`, `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `SUPABASE_TABLE_PREFIX=nb_`, `SUPABASE_STORAGE_MODE=normalized`를 설정합니다.
3. 재배포 후 `/api/health`의 `runtime.providerDiagnostics.storage.missingEnv`가 비어 있는지 확인합니다.
4. 운영자 세션에서 `GET /api/admin/storage-check`를 호출하고 `productionGate.ready=true`, `productionGate.schemaReady=true`인지 확인합니다.
...
3. Render Dashboard > Web Service > Environment에 `SMS_PROVIDER=solapi`, `SOLAPI_API_KEY`, `SOLAPI_API_SECRET`, `SOLAPI_SENDER_NUMBER`, `PHONE_CODE_HIDE_DEBUG=true`를 설정합니다.
4. `/api/health`에서 `smsProvider: "solapi"`, `smsConfigured: true`, `phoneDebugCodeExposed: false`, `runtime.providerDiagnostics.sms.missingEnv: []`인지 확인합니다.

`SMS_PROVIDER=dev` 또는 `PHONE_CODE_HIDE_DEBUG=false`는 개발용입니다. production readiness에서는 이 상태를 blocker로 표시하고, Solapi 키/secret/발신번호 중 빠진 값은 `runtime.providerDiagnostics.sms.missingEnv`에 이름만 노출합니다. 실제 전송 실패는 사용자에게 내부 provider 오류 본문을 보여주지 않고 일반 SMS 발송 실패로 응답하며, 재요청/인증 시도 제한은 `PHONE_CODE_RESEND_SECONDS`, `PHONE_CODE_MAX_ATTEMPTS`, 인증 관련 rate limit 값으로 조정합니다.
...
운영 readiness와 `/api/health`는 Supabase env 누락을 `missingEnv`로 표시합니다. `storage-check`는 실제 정규 테이블을 조회해 `productionGate`에 env/schema/mode 준비 상태와 실패한 테이블을 요약합니다.
